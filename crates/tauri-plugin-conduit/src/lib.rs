...
impl<R: Runtime> PluginState<R> {
    ...
    fn validate_invoke_key(&self, candidate: &str) -> bool {
        ...
        let ok: bool = self.invoke_key_bytes.ct_eq(&candidate_bytes).into();
        ok
    }
}
...
impl PluginBuilder {
    ...
    // -- Context-aware handlers (#[command]-generated) -----------------------

    /// Register a `#[command]`-generated handler.
    ///
    /// Handlers generated by the `#[conduit::command]` macro have the signature
    /// `fn(Vec<u8>, &dyn Any) -> Result<Vec<u8>, Error>` and handle their own
    /// deserialization, State extraction, and serialization internally.
    ///
    /// ```rust,ignore
    /// use conduit::command;
    ///
    /// #[command]
    /// fn greet(state: State<'_, AppState>, name: String) -> String {
    ///     format!("Hello {} from {}", name, state.app_name)
    /// }
    ///
    /// tauri_plugin_conduit::init()
    ///     .handler("greet", greet)
    ...
    pub fn channel_ordered(mut self, name: impl Into<String>) -> Self {
        self.channel_defs
            .push((name.into(), ChannelKind::Ordered(DEFAULT_CHANNEL_CAPACITY)));
        self
    }
            ...
            .register_uri_scheme_protocol("conduit", move |ctx, request| {
                ...
                if segments.len() != 2 {
                    return make_response(
                        404,
                        "text/plain",
                        b"not found: expected /invoke/<cmd> or /drain/<channel>".to_vec(),
                    );
                }
                    ...
                    Some(v) => match v.to_str() {
                        ...
                        Err(_) => {
                            return make_response(
                                401,
                                "text/plain",
                                b"invalid invoke key header".to_vec(),
                            );
                        }
                    },
                    None => {
                        return make_response(401, "text/plain", b"missing invoke key".to_vec());
                    }
                };
                ...
                match action {
                    "invoke" => {
                        ...
                        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                            dispatch.call_or_error_bytes_with_context(target, body, &app_handle)
                        }));
                    ...
                    "drain" => {
                        ...
                        match state.channel(target) {
                            ...
                            None => make_response(
                                404,
                                "text/plain",
                                format!("unknown channel: {target}").into_bytes(),
                            ),
                        }
                    }
                    _ => make_response(
