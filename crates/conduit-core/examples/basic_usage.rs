...
//!
//! 1. Creating a DispatchTable and registering handlers
...
use conduit_core::{
    ConduitRingBuffer, DispatchTable, FRAME_HEADER_SIZE, FrameHeader, MsgType, PROTOCOL_VERSION,
    WireDecode, WireEncode, frame_unwrap, frame_wrap,
};
...
fn main() {
    ...
    // -----------------------------------------------------------------------
    // Step 1: Create a DispatchTable and register handlers
    // -----------------------------------------------------------------------
    println!("--- Step 1: DispatchTable ---\n");
    ...
    table.register("greet", |payload: Vec<u8>| {
        match String::wire_decode(&payload) {
            Some((name, _consumed)) => {
                let greeting = format!("Hello, {name}! Welcome to conduit.");
                let mut out = Vec::new();
                greeting.wire_encode(&mut out);
                out
            }
    ...
    table.register_simple("version", || {
        let mut out = Vec::new();
        String::from("conduit-core 0.1.0").wire_encode(&mut out);
        out
    });
    ...
    String::from("Alice").wire_encode(&mut payload);
    ...
    let request_frame = frame_wrap(&request_header, &payload);
    println!(
    ...
    let response_frame = frame_wrap(&response_header, &response_bytes);
    println!("  Response frame: {} bytes total", response_frame.len());

    // Verify roundtrip: unwrap the response frame.
    let (resp_hdr, resp_payload) = frame_unwrap(&response_frame).unwrap();
    assert_eq!(resp_hdr.msg_type, MsgType::Response);
    assert_eq!(resp_hdr.sequence, 1);
    let (decoded_greeting, _) = String::wire_decode(resp_payload).unwrap();
    ...
    let ring = ConduitRingBuffer::new(1024);
    ...
    for i in 0u32..5 {
        ...
        i.wire_encode(&mut tick_payload);
        (100.0 + i as f64).wire_encode(&mut tick_payload);
        ...
        let tick_frame = frame_wrap(&tick_header, &tick_payload);
    ...
    for i in 0..frame_count {
        ...
        let (hdr, payload) = frame_unwrap(frame_bytes).expect("bad frame in drain blob");

        // Decode the tick payload: u32 index + f64 price.
        let (index, consumed) = u32::wire_decode(payload).unwrap();
        let (price, _) = f64::wire_decode(&payload[consumed..]).unwrap();
